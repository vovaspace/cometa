...
describe("container", () => {
	...
	describe("resolution scope", () => {
		...
		it.each<{
			...
			factory: (instance: unknown) => unknown;
			pure: boolean;
		}>([
			{
			...
			{
				name:
					"a synchronous instance with already resolved asynchronous dependencies",
				sync: true,
			...
			if (hasDependencies)
				container.bind(c.name, {
					...
					dependencies: () =>
						Object.fromEntries(c.dependencies.map((d) => [d.key, d])) as {
							_: never;
						},
					cleanup: () => {},
					pure: c.pure,
			...
			if (hasDependencies)
				expect(resolved.instance).toStrictEqual({
					dependencies: Object.fromEntries(
						c.dependencies.map((d) => [d.key, instances.get(d)]),
					),
				});
			else expect(resolved.instance).toStrictEqual({ empty: true });
		});
