...
import { event, type Event, type EventProtocol } from "./event";
...
function createEffect<Payload = void, Result = void, Err = Error>(
	...
	const launched = event<EffectLaunchedPayload<Payload>>();
	const fulfilled = event<EffectFulfilledPayload<Payload, Result>>();
	const rejected = event<EffectRejectedPayload<Payload, Err>>();
	const settled = event<EffectSettledPayload<Payload, Result, Err>>();
	const payloaded = event<Payload>();
	const resulted = event<Result>();
	const failed = event<Err>();
